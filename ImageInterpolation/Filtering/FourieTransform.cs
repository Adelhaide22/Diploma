...
namespace ImageInterpolation.Filtering
{
	...
	{	
		public static Complex<double>[,] TransformForward(double[,] f)
		{
			var height = f.GetLength(1);
			var F = new Complex<double>[f.GetLength(0), f.GetLength(1)];
		...
		public static double[,] TransformBackward(Complex<double>[,] F)
		{
			var height = F.GetLength(1);
			...
			for (int k = 0; k < F.Length; k++)
			{
				...
				f[k / height, k % height] = temp.Magnitude;
			}
